...
data_indx = 0
datasets_names = []
for i, (root, dirs, files) in enumerate(os.walk("results/knn")):
    for filename in files:
        filepath = root + os.sep + filename
        if filepath.endswith("knn.npy"):
            print(filename)
...
np.save("dataset_names_knn", datasets_names)
...
np.save("gathered_knn", gathered_by_div)
exit()
# """
...
np.save("gathered_knn_preproc", gathered_preproc)
# """
